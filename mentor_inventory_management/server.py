...
def stores_list():
    ...
    data = Store.select(Store.name, Store.id, fn.Count(Warehouse.id).alias(
        'num')).join(Warehouse, JOIN.LEFT_OUTER).group_by(Store.id).order_by(Store.id)
...
def warehouse_update(id):
    w = Warehouse(
        id=id, location=request.form['location'], store=request.form['store_id'])
...
def product_create():
    p = Product(name=request.form['name'], description=request.form['desc'],
                color=request.form['color'], warehouse=request.form['warehouse_id'])
    if p.save():
        flash("Successfully saved", "success")
    else:
...
def product_update(id):
    p = Product(id=id, name=request.form['name'], description=request.form['desc'],
                color=request.form['color'], warehouse=request.form['warehouse_id'])
